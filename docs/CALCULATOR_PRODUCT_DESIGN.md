...
It accepts exact JSON for each currently recovered event transition and exposes
the property and counter state before and after the event. This makes the rules
usable for manual sequencing while keeping them separate from damage output.
Unsupported Wheels and incomplete inputs fail rather than receiving neutral
defaults.
