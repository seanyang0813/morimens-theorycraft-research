...
Agents can invoke the same boundary as `calculate-snapshot-active-damage` through `engine/theorycraft-api.mjs` or `tools/run_theorycraft_request.mjs`. `research/examples/theorycraft-current-snapshot-active.json` is an explicitly synthetic resource-150 contract example. It demonstrates the input shape and read trace; its values are not a character build or gameplay evidence.
