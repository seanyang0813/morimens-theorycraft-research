...
`tools/freeze_blind_replay_prediction.mjs` scans actions and complete direct-hit boundaries in chronological order, selecting the first deterministic Active candidate whose action contains no prior hit. A later hit after an earlier hit cannot reuse card-start HP or Block without learning the earlier outcome, so it is blocked even if its own critical branch is deterministic. The freezer replaces the candidate hit with an identity-only marker before calculation. Target, caster and skill identity are disclosed conditions of the prediction; target selection itself is not predicted. Recorded damage, critical flag, old HP, Block loss and HP-loss fields are excluded. Target HP and Block come from the card-use boundary, with an omitted Block property interpreted as the original getter's zero default. The public evidence contains only the replay container hash, catalog hashes, an identity commitment, scenario inputs and the frozen result; player and replay identifiers remain private.
