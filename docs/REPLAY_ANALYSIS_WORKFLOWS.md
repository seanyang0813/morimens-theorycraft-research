...
The current private inventory covers 69 captures and contains 13 same-coordinate structural groups. The largest groups contain 13 and 11 captures. The independent domain audit classifies the newest 27 captures as `PVP_PLAYER_TARGETS`; the other 42 have no imported domain decision in this inventory. Therefore the current D-Tide-eligible count and PvE frontier count are both zero. One same-coordinate group contains unusually low investment signals, but it remains excluded: it is in the audited Player-target corpus, and the shipped client tables also do not map its dynamic server stage ID to a user-facing D-Tide difficulty label. It is not D-Tide budget, cheese or optimal-build evidence.
