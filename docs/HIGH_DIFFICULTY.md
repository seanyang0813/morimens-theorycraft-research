...
When skill 60397 (“Final Evolution”) later executes, its exact command 60401 applies 20 layers of permanent Reinforce state 60089, applies `ceil(ATK × 0.08)` layers of Strength state 2900, records a `BEMonsterBubble` row, and installs listener state 60404. `BEMonsterBubble.DoEffect` has been executed over six eligibility/default cases in both the baseline and installed resource-150 runtimes: after its superclass call it only validates the tip and Monster target, then calls `recordMgr:OnMonsterBubble`; it does not mutate combat state in that body. The two module files are byte-identical. The listener responds only to a later `BSTHpChanged` event whose signed `TriggerValue` is negative. Command 60402 then adds two layers of temporary Reinforce state 60083. This is tied to actual HP decrease events rather than every attempted or fully blocked hit.
...
Forty-two authorized server replay containers are now preserved privately. Their indexes contain 1,804 complete hit snapshots. The strict adapter accepts 428 supported Active-hit candidates: all 269 deterministic calculations match exactly with zero mismatches, and all 159 chance-dependent cases match one independently evaluated roll-1 or roll-100 branch. Candidate identity now requires the recorded caster and skill to match the played card, preventing adjacent state-trigger damage from being assigned to that card. Every candidate routes through replay-embedded combat data; catalog attribution separately identifies 35 resource-144 matches, 3 resource-150 matches and 4 unmatched or intermediate catalogs. The selected Mouchette rows remain behavior-identical to current client rows after normalizing Lua lists and excluding `BaseSortID` ordering metadata. This audit is verification-track regression evidence only. It does not identify cheese, rank budget teams or produce theorycraft recommendations, and it receives no publication or holdout credit because outcomes were recovered before calculation, predictions were not frozen before a separate reveal, original critical draws were not captured, and replay payloads contain no explicit engine-code version.
