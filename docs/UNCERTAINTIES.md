...
4. **HP resolution:** schema 2 complete-property snapshots now model immunity, shield/Puncture, retained-HP thresholds, damage budgets, death resistance and ordinary HP subtraction through the runtime-matched subset. Callback execution, statistics, death scheduling and independent gameplay validation remain unresolved, so this is reported as `modeledHpLost` while `finalDamage` remains null.
