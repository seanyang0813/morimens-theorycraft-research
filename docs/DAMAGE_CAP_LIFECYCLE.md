...
Five joined transition cases now execute every relevant compiled condition and parameter against the layers left by preceding original effect bodies. A Python row iterator invokes original `BEAddState`, `BESubStateLayer`, `BERemoveState` and `BEMonsterChangeSkill` bodies over one original manager registry. Nontransition, exact-boundary and over-boundary paths for both 60406 and 60405 match `resolvePhaseHpLoss`, including counter saturation, retain-one behavior, immunity, skill selection, second-phase installation and final removals. Current resource-150 expressions and its changed add-state parent reproduce all five. The destination states are pre-created zero-layer adapters in this joined probe; the separate creation/property probes establish their absent-state paths. The original command parser and scheduler still do not join these pieces automatically.
...
Next verification needs the original command parser/scheduler connection, transition-skill component internals and independent high-difficulty observations. These synthetic fixtures do not change the publication gate.
