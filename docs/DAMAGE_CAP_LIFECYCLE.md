...
First-time creation is now connected separately. Sixteen cases route absent states 46441, 60407, 60408 and 60409 from `BEAddState` through original manager creation, original `BattleStateServer.ctor`, `BattleStateData.Create`, registration and `AfterInit`, using each catalog `MaxLayer`. Five additional cases continue absent counter 60407 through original `InitProperty` and original `BattlePropertyServer` mutation: a requested `ChangedLayer` of 1, 2, 125, 330 or 999999999 becomes the stored `be_damage_statics` contribution and fires the owner/send callbacks before serialization and manager add events. The installed resource-150 `BEAddStateParent` and `BattlePropertyServer` reproduce these results exactly. The constructor callable, command-expression values, target and callback endpoints remain explicit adapters; command conditions and event bodies are not part of this connection.
...
Next verification needs the joined command-condition/effect sequence, phase-state property initialization, transition-skill internals and independent high-difficulty observations. These synthetic fixtures do not change the publication gate.
