...
`tools/active_effect_execution_oracle.py` now connects original effect-manager construction to original `BEActiveDamage` and `BEFunctionEffect` constructors, target/parameter binding, repetition routing, function dispatch, `Damage2SingleTarget`, original `GetRealDmg`, original `BattleUnitBase.BeHit`, original `BattlePropertyServer` HP mutation and original nonlethal HP/damage event-request construction. Four positive-repeat cases confirm parent/time capture, repeat calculation and timing, child creation, per-child parameter regeneration, critical scaling, hit-result construction, cumulative HP loss, and the per-hit request order `RoleHpChanged` → `RoleHpperChanged` → `DoDamage` → `BeDamage`. The cases use explicit neutral PvE actor/player/target adapters and empty skill types. Property-send, hit-record and event-factory callbacks are observers; animation, listener execution and the death path remain outside this boundary.
