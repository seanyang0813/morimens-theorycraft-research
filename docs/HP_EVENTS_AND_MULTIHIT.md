...
`tools/active_effect_execution_oracle.py` now connects original effect-manager construction to original `BEActiveDamage` and `BEFunctionEffect` constructors, `PreTrigger`, `TryDoEffect`, parameter/target binding, initialization, repetition routing, function dispatch, `Damage2SingleTarget` and original `GetRealDmg` offensive/final-target arithmetic. Four positive-repeat cases confirm parent/time capture, flat and percentage repeat calculation, first-hit versus later-hit delay, skip-phase delay suppression, child creation per target, per-child parameter regeneration, critical scaling and the critical flag forwarded to the hit request. The formula cases use explicit neutral PvE actor/player/target adapters and empty skill types. `BeHit` still records the request, so connecting the original HP path remains the next boundary.
