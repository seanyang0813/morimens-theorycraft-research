...
`tools/active_effect_execution_oracle.py` now connects original effect-manager construction to original `BEActiveDamage` constructors, `PreTrigger`, `TryDoEffect`, parameter/target binding, initialization and one or two repetition-routing calls. Four positive-repeat cases confirm parent/time capture, flat and percentage repeat calculation, first-hit versus later-hit delay, skip-phase delay suppression, and child creation per target. The child `BEFunctionEffect` is inert, so `Damage2SingleTarget`, the formula, `BeHit` and HP mutation remain the next boundary.
