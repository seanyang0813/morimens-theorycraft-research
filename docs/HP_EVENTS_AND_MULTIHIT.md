...
`tools/active_effect_execution_oracle.py` now connects original effect-manager construction to original `BEActiveDamage` and `BEFunctionEffect` constructors, `PreTrigger`, `TryDoEffect`, parameter/target binding, initialization, repetition routing and function dispatch into original `Damage2SingleTarget`. Four positive-repeat cases confirm parent/time capture, flat and percentage repeat calculation, first-hit versus later-hit delay, skip-phase delay suppression, child creation per target, per-child parameter regeneration and the critical flag forwarded to the hit request. `GetRealDmg` is still an identity observer and `BeHit` records the request, so connecting the real formula and original HP path remains the next boundary.
