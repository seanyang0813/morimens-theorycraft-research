...
`tools/active_effect_execution_oracle.py` now connects original effect-manager construction to original `BEActiveDamage` and `BEFunctionEffect` constructors, target/parameter binding, repetition routing, function dispatch, `Damage2SingleTarget`, original `GetRealDmg`, original `BattleUnitBase.BeHit` and original `BattlePropertyServer` HP mutation. Four positive-repeat cases confirm parent/time capture, repeat calculation and timing, child creation, per-child parameter regeneration, critical scaling, hit-result construction and cumulative HP loss. The cases use explicit neutral PvE actor/player/target adapters and empty skill types. Property callbacks and hit recording are observers; animation and downstream damage/death event dispatch remain no-op adapters, so this is not yet a complete event lifecycle.
