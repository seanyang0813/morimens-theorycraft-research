...
`tools/active_effect_execution_oracle.py` now connects original effect-manager construction to original `BEActiveDamage` and `BEFunctionEffect` constructors, target/parameter binding, repetition routing, function dispatch, `Damage2SingleTarget`, original `GetRealDmg`, original `BattleUnitBase.BeHit`, original `BattlePropertyServer` HP mutation and original HP/damage event-request construction. Five positive-repeat cases confirm parent/time capture, repeat calculation and timing, child creation, per-child parameter regeneration, critical scaling, hit-result construction and cumulative HP loss. Nonlethal hits request `RoleHpChanged` → `RoleHpperChanged` → `DoDamage` → `BeDamage`. The lethal monster case applies a 100-damage hit at 50 HP, reports 50 real damage and 50 overflow, requests `BERoleDeadlyDamage` then `BERoleDie`, and then requests `ActiveDamageKill`. The cases use explicit neutral PvE actor/player/target adapters and empty skill types. The two death-effect constructors are request observers only. Property-send, hit-record and event-factory callbacks are observers; animation, listener execution and death-effect bodies remain outside this boundary.
...
Resource 150 keeps `BERoleDeadlyDamage` byte-identical but changes `BERoleDie` and `BattleUnitMonster`. The selected runtime comparison shows the refactor delegates PvE and first-free-respawn eligibility to actual current role methods while preserving eight tested outcomes: missing role, revival before death, monster death, two player-confirm branches, intro auto-respawn, PvP death and monster pre-check eligibility. `OnConfirm`, `RoleDie` internals and full scheduler traversal remain separate work.
