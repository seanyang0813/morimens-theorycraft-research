...
The composition uses the command-row scheduler and independently probed damage/energy components. The complete mixed command has not been run as a connected original-runtime comparison. It omits card costs, lifecycle, callbacks, state effects, presentation and timing by explicit assumption. All targets/build attributes are supplied, not automatically reconstructed. The original three-effect skill with BEAddState remains unsupported. Source IDs are explicit; the runner does not claim automatic skill-to-command provenance. The JSON CLI is also exposed in the local unpublished Actions workbench; it is not deployed.
