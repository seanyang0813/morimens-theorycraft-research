...
The Rules catalog now contains 275 entries. Its Wheel mechanics entry reports
...
The agent CLI now also exposes the installed `PlayerRole.tentacle_dmg` producer as a separate resolved-input operation. Its 377 synthetic runtime matches and 19 exact retrospective pre-hit comparisons establish a narrow upstream input and component check, not an independently frozen prediction. The general browser form does not yet expose this producer or assemble a Tentacle build automatically.
