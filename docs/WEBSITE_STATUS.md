...
`website/dist/tentacle.html` adds a general direct-Tentacle input view for the installed resource-151 client. It calls the same `calculateDirectTentacleCommand` engine used by the agent API, accepts one or more explicit Awakener property maps, and shows Player production, command ceiling, regional critical bonus and target pre-hit damage. The bundled example demonstrates the input contract; it is not a reconstructed build or gameplay observation. Target-state eligibility, critical RNG, BeHit/HP, repeats and independent validation remain unresolved.

The Property Snapshots page accepts complete live resource-144, resource-150 or bounded resource-151 caster, player, target and optional card property maps. It runs the verified local snapshot adapter, displays pre-hit damage and exposes every property read, including original getter zero defaults. Its crit comparison evaluates rolls 1 and 100 from the same snapshot, reports both damage branches, the one-hit critical probability under the recovered draw rule and the weighted expectation. It does not predict the actual RNG draw or a multi-hit distribution. Schema 2 also displays modeled HP loss after immunity, shield/Puncture, retained-HP, incoming-limit and death-resistance handling. Its synthetic installed-client resource-151 single-hit example renders 317 pre-hit damage, consumes 50 shield, reports 267 modeled HP loss and records 79 property reads. Its installed-client sequence example threads only HP and Block across two hits and reports 150 modeled HP loss. Partial maps and unsupported branches fail closed; callbacks, death execution, independent gameplay verification and verified `finalDamage` remain unresolved.
...
`tools/prepare_local_website.py` prepares 142 explicitly allowlisted authored engine modules plus sanitized catalogs and page assets. It does not publish original Lua, native libraries, keys, screenshots, replay observations or private research directories. The verification suite checks copied engine modules byte-for-byte against their authored sources, along with static references, JavaScript syntax and calculation integrations.
...
The Rules catalog now contains 278 entries. Its Wheel mechanics entry reports
