...
A separate [narrow replay-adapter dependency audit](../research/evidence/pc-res151-to-res153-replay-adapter-dependencies.json) checks the fields actually consumed when routing comes from replay-embedded Skill/Cmd rows. Its six code modules and BattleApi table are unchanged; the existing 6,959 State rows retain their `IsBuff` classification, the one added state classifies as neither buff nor debuff, and Constant's `DamagePer2HasState` mapping is unchanged. Run `python tools/audit_res153_replay_adapter_dependencies.py` against the ignored exports to reproduce it. This identifies a possible resource-153 carry-forward path, but the engine and blind freezer still reject resource 153, and no gameplay validation is claimed.
...
This run confirms that a completed resource-153 story battle can yield a recoverable PvE replay through the current capture and indexing pipeline. Damage and result screens were visible during play, the run contained multiple battles, and the resource-153 damage adapter remains unsupported. This is a retrospective capture diagnostic with **zero blind holdouts** and no publication credit.
