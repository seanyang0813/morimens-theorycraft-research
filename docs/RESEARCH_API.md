...
`calculate-direct-tentacle-command` composes those two property-derived stages with the exact resource-151 ordinary PvE `BETentacleAttack` row `PlayerRole.tentacle_dmg*CmdCaster.occupation_master/200,1,0` and the resolved target pre-hit formula. It requires all Player, Awaker, caster occupation, region, critical-outcome and target inputs explicitly; the exact row's third parameter must be zero. The response shows the Player value, effect ceiling, conditional critical bonus and final pre-hit trace. The sanitized example yields 230 -> ceiling(230×92/200)=106 -> ceiling(106×1.84×1.2)=235. This composition agrees with 39 outcome-first retrospective staged comparisons; it is not an independent gameplay prediction, does not select a skill/target or resolve critical RNG, and does not calculate HP damage. See `research/examples/theorycraft-installed-direct-tentacle-command.json` and `research/evidence/tentacle-replay-simple-consistency.json`.

`aggregate-tentacle-awaker-bonuses` accepts one to four Awakeners with already resolved target-category bonuses and named, eligible target-state properties. The installed Tentacle method averages five categories over the Awakeners. Its shared state-property table adds contributions with the same name; the resulting named totals multiply as separate factors. The operation returns five averaged percentages and a direct `enemyStateMultiplier` for the `calculate-damage` Tentacle path; that path accepts either the legacy resolved `enemyStatePer` percentage or this direct multiplier, never both. The rule matches 346 copied original-runtime executions and selected installed method-body parity, not gameplay. See `research/examples/theorycraft-installed-tentacle-team-bonuses.json` and `research/evidence/pc-res151-tentacle-team-aggregation.json`.
