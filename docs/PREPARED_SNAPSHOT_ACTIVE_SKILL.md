...
The bridge is deliberately strict. It accepts exactly one unconditional `BEActiveDamage` row aimed at `UpperTarget`, ordinary subtype `0`, no `ParaPlus`, one explicitly supplied target, and an explicit critical-roll entry for every derived hit. The selected catalog `CmdTarget` expression must match the caller's target binding. Mixed commands, unsupported subtypes, missing random outcomes, and extra caller fields fail before calculation.
