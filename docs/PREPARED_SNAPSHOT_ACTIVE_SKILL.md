...
Schema 4 handles a mixed command conservatively. It executes only the contiguous leading `BEActiveDamage` rows, preserves the first unsupported row as `stop`, and never skips that row to reach later effects. A catalog selector such as `AllEnemy` may be bound to the supplied snapshot only when `eligibleTargetCount` is explicitly `1`; otherwise the request fails. This makes the direct-hit portion of current Mouchette Mortal Blast (Skill 122483 / Cmd 122499) calculable from complete live properties while leaving its card-copy boundary visible.

`run-prepared-mortal-blast` now joins that prefix to the separately tested copy suffix. It verifies Skill 122483, Command 122499, five catalog rows, the row-2 stop and catalog-derived `Arg2 > 1`; then it composes explicit history selection, top-of-hand mutation, returned-card `LastTarget` and the three Card-state properties. This is still not a full Mortal Blast sequence or damage total. It supports one eligible enemy, does not execute one connected scheduler chain, and does not play the generated Strike.

The schema-1 bridge is deliberately strict. It accepts a catalog-tagged Awakener damage card with exactly one unconditional `BEActiveDamage` row aimed at `UpperTarget`, ordinary subtype `0`, one explicitly supplied target, and an explicit critical-roll entry for every derived hit. Damage tags and ordinary card context come from the selected Skill row rather than caller input. When the selected Skill has `ParaPlus`, the bridge evaluates that catalog expression from the same explicit variable/state-query environment, creates `ParaPlusN` bindings, and sends the resolved fourth Active parameter through `skillArgsPlus` on every hit. The selected catalog `CmdTarget` expression must match the caller's target binding. Monster intents, unsupported subtypes, missing expression inputs, missing random outcomes, and extra caller fields fail before calculation. Mixed commands require one of the explicitly bounded later schemas.
...
The installed resource-151 bounded example is:

```text
node tools/run_theorycraft_request.mjs --input research/examples/theorycraft-installed-prepared-snapshot-active.json
```
