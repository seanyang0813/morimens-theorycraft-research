...
4. Select a deterministic isolated action. Confirm hit count and critical eligibility from evidence, not the desired output. Save the scenario, pre-action evidence and separate evidence showing the executing client build, then run `tools/freeze_gameplay_prediction.mjs --recorded-build pc-res144-build51 --build-evidence <file>` before inspecting the result. The tool pins the scenario, runtime, prediction, build identity and evidence hashes and refuses overwrite.
