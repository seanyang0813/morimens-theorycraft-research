...
A separate filtered statistics view gives stronger aggregate evidence: boss turn 1 credits 阿拉克涅 with Strike 2205 and Old Embers 6615 (total 8820). The executable retrospective mechanic check reproduces 6615 exactly from the observed Strike `castDamage`, sufficient resource and the recovered 3x Active-trigger rule. The recorded combat build, pre-hit Strike inputs and actual HP loss remain unknown; this supports one exact downstream consistency regression, not a verified full damage prediction or holdout.
