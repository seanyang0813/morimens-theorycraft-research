...
This property assembly matches 256 executions of original resource-144 `BattleCmdServer.GetRealBlock`. The same 256 fixtures match the installed resource-150 constants, utility and command bytecode exactly. Resource 151 directly matches all 256 calculation fixtures and all 160 Block-storage fixtures, then uses the exact-carried 516-case energy boundary and Lua-semantically equal catalogs. The public composition report is `research/evidence/pc-res151-prepared-block-compatibility.json`. The two-row command composition remains authored component integration: target generation, card legality and payment, effect repetition, callbacks, Block events and later actions do not execute.
...
Use `research/examples/theorycraft-installed-prepared-snapshot-block-skill.json` for the installed resource-151 build.
