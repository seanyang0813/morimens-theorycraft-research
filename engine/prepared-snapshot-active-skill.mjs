...
export function runPreparedSnapshotActiveSkill(value,source){
  ...
  if(mixed){
    if(!exact(input.energy,['source','target'])||!exact(input.energy.source,['castRoleUid','cmdServerUid'])||!exact(input.energy.target,['uid','role','energy','maximumProperties','calculation'])||!exact(input.energy.target.calculation,['dimension','properties','casterEligible'])||input.energy.source.castRoleUid!==input.energy.target.uid)throw new Error('Explicit self-target Awakener energy context required');
