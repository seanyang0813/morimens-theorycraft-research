...
function wheelInput(input,wheelState,targetProperties,steps){
  return {schemaVersion:1,kind:'morimens-wheel-active-timeline',build:input.build,snapshotStage:input.snapshotStage,snapshotCompleteness:input.snapshotCompleteness,initialWheelState:wheelState,baseCasterProperties:input.baseCasterProperties,basePlayerProperties:input.basePlayerProperties,initialTargetProperties:targetProperties,steps};
}
...
export function runPaidWheelActiveTimeline(value){
  const input=clone(value),keys=['schemaVersion','kind','build','initialEnergy','snapshotStage','snapshotCompleteness','initialWheelState','baseCasterProperties','basePlayerProperties','initialTargetProperties','actions'];
  if(!exact(input,keys)||input.schemaVersion!==1||input.kind!=='morimens-paid-wheel-active-timeline'||input.build!=='pc-res144-build51'||!Array.isArray(input.actions)||input.actions.length===0)throw new Error('Exact resource-144 paid Wheel Active timeline required');
  ...
  for(const action of input.actions){
    ...
    if(input.initialWheelState.doomsday)allEffects.push({id:`${action.id}:after-use`,type:'AFTER_USE_CARD',cardType:action.cardType});
  ...
  runWheelActiveTimeline(wheelInput(input,input.initialWheelState,input.initialTargetProperties,allEffects)); // validates every effect suffix
  let energy=input.initialEnergy,target=clone(input.initialTargetProperties),wheelState=privateWheelState(input.initialWheelState),stop=null,acceptedActions=0;
  ...
  for(const action of input.actions){
    ...
    const effect=runWheelActiveTimeline(wheelInput(input,wheelState,target,effectSteps(action,wheelState.doomsday!==null)));row.effect=effect;effect.unresolvedDependencies.forEach(item=>dependencies.add(item));
    target={...target,hp:effect.targetAfter.hp,block:effect.targetAfter.block};wheelState=privateWheelState(effect.finalWheelState);
    if(!effect.completed){stop={actionId:action.id,phase:'effects',detail:effect.stop};break;}
  }
  return {schemaVersion:1,kind:'morimens-paid-wheel-active-timeline-result',analysisTrack:'theorycrafting',status:'EXPERIMENTAL_COMPOSITION',build:input.build,finalDamage:null,completed:stop===null,stop,initialEnergy:input.initialEnergy,energyAfter:energy,modeledEnergyLost:input.initialEnergy-energy,acceptedActions,unattemptedActions:input.actions.length-trace.length,initialTarget:{hp:input.initialTargetProperties.hp??0,block:input.initialTargetProperties.block??0},targetAfter:{hp:target.hp??0,block:target.block??0},modeledHpLost:(input.initialTargetProperties.hp??0)-(target.hp??0),finalWheelState:publicWheelState(wheelState),trace,unresolvedDependencies:[...dependencies,'Ordinary PvE play checks and payment are composed before each accepted action; rejected actions expose no damage or Wheel event','Doomsday after-use is appended after all supplied effects of an accepted card; exact multi-listener scheduler ordering and lethal-card after-use delivery remain unresolved','Each action supplies complete Active-hit snapshots and may explicitly place Arachne post-pursuit events; pursuit generation, target selection and other events are not inferred','Hand removal, draw, refunds, changing costs/conditions, turn transitions, death execution and gameplay validation remain unresolved']};
}
