...
export function activeTargetDamage(showDamage,data,build='pc-res144-build51',{cardPresent=false}={}){
  if(!['pc-res144-build51','pc-res150-build51','pc-res151-build51','pc-res153-build51'].includes(build))throw new Error('Unsupported target-damage build');
  ...
  const evidence=build==='pc-res144-build51'?'PC144:BattleCmdServer.__GetFinalDamage':build==='pc-res150-build51'?(cardPresent?'PC150:BattleCmdServer.CardTargetCrit':'PC150:BattleCmdServer.FinalTargetDamage'):(cardPresent?`PC${build==='pc-res153-build51'?'153':'151'}:BattleCmdServer.CardTargetCrit.ExactDependencyCarryforward`:`PC${build==='pc-res153-build51'?'153':'151'}:BattleCmdServer.FinalTargetDamage.ExactDependencyCarryforward`);
