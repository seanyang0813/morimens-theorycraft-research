...
export function resolveImmunity(input) {
  ...
  if(!['pc-res144-build51','pc-res150-build51'].includes(input.build)||!categories.includes(input.category))throw new Error('Unsupported immunity build or category');
  ...
  return {immune:punctureBlocked||categoryBlocked,
    ...
    evidence:[input.build==='pc-res150-build51'?'PC150:BattleUnitBase.BeHitHp':'PC144:DamageImmunity'],status:'UNVERIFIED',finalDamage:null};
}
