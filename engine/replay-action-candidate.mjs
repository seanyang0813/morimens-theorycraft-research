...
export function buildReplayActionCandidate({index,actionIndex,hitIndex=null,skills,commands,monsters,awakeners,battleApi=null,critRoll=null,preOutcome=false,combatBuild=protocolBuild}){
  ...
  const tagSelection=selectProgressionList({value:skill.Type??null,breakSkillLevel,potencyLevel});
  if(!tagSelection.value)throw new Error('Played skill type tags must resolve at captured progression');
  const tags=[...tagSelection.value];
  const strikeFix=card.properties?.card_type_strike??0;
  if(!Number.isFinite(strikeFix))throw new Error('Captured card Strike-type override must be finite');
  if(strikeFix>0&&!tags.includes('Card_Strike'))tags.push('Card_Strike');
