...
export function calculateSnapshotActiveDamage(value){
  ...
  const targetData={isCrit:context.isCrit,awakerCritDamage:read('caster','crit_damage'),cardCritDamage:input.cardContext.present?read('card','crit_damage'):0,
    ...
    enemyBlockBarrierDmgPer:targetBlock>0||context.targetBlockBarrierStatePresent?read('caster','damage_per2block_barrier'):0,cardBlockBarrierPer:input.cardContext.present&&(targetBlock>0||input.cardContext.blockBarrierStatePresent)?read('card','card_damage_per2block_barrier'):0,
