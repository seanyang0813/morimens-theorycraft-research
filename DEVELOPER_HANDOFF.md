...
The general runners can model resolved Active, Passive, Fixed and Pure hits; HP limits; energy and card payment; bounded multi-hit timelines; selected state lifecycle/property effects; and some imported command shapes. The current command frontier is a damage/energy prefix followed by one unconditional terminal `BEAddState` on the caster. See `engine/terminal-state-command.mjs`.

The next valuable engine work is to connect that terminal-state shape through prepared skills and the Actions workbench, then support later rows observing applied states without weakening the strict unsupported-effect checks. Broader command/state execution, automatic build assembly, encounter phases and reactive event graphs remain incomplete.
