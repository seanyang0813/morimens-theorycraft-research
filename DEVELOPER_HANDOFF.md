...
The completed H1-7 and H1-8 Hard account replays are privately preserved with same-process, post-baseline provenance. Their identifier-free retrospective reports ([H1-7](research/evidence/res153-hard-h1-7-own-pve-prehit-retrospective-20260923.json), [H1-8](research/evidence/res153-hard-h1-8-own-pve-prehit-retrospective-20260923.json)) record 17 and 19 exact direct-card Active pre-hit matches selected with observed critical flags. The H1-8 audit excludes 53 other-caster/skill hits and seven unsupported hit or command shapes. This is regression coverage, not a blind holdout or a full-battle simulation. `tools/sanitize_res153_story_prehit.py` reproduces the reports from the ignored private captures after checking provenance, account, stage, domain and audit hashes.
...
Resource 151 is supported by the narrow replay-embedded, complete-live-property Active adapter and four matching agent-CLI operations: snapshot damage, repeated snapshot damage, catalog-prepared snapshot Active damage, and preparation-only skill inspection. The CLI also exposes `run-ulti-energy-effect` through a separate seven-module equality boundary carrying 516 resource-150 runtime fixtures, `run-prepared-snapshot-block-skill` after 416 direct resource-151 Block matches plus the unchanged catalogs and energy boundary, the singular `run-prepared-state-active-sequence` after 432 direct resource-151 state/property mutation matches plus the unchanged catalogs and Active boundary, its schema-1/4 `run-prepared-state-active-chain` composition, `run-paid-prepared-state-active-chain` through seven byte-identical legality/payment modules carrying 90 play-check plus 222 payment fixtures, and nine Wheel event/damage/payment/search operations through unchanged Wheel rows plus those Active/payment boundaries; see `pc-res151-ulti-energy-compatibility.json`, `pc-res151-prepared-block-compatibility.json`, `pc-res151-state-active-sequence-compatibility.json`, `pc-res151-paid-state-active-chain-compatibility.json` and `pc-res151-wheel-timeline-compatibility.json`. `research/evidence/pc-res151-replay-adapter-compatibility.json` composes the exact six-module replay boundary with five Lua-semantically equivalent config tables. A resource-151 replay prediction freeze must carry that report, `pc-res144-to-res151-combat-build.json` and reviewed same-session capture evidence matching the exact container. Do not widen other state chains, card-manager, lifecycle or general theorycraft paths without a complete dependency audit. The general role-state API remains closed on resource 151; the internal prepared setup is exposed only through the singular state-to-Active operation and its schema-1/4 ordered and paid compositions.

Resource 151 also has a separately validated build-planner boundary. The eight installed progression modules are byte-identical to resource 150; its own original GetAwakerBaseAttrValue matches 2,700 checked character/level/Gnostic/stat cases. client-build-data-res151.json is a sanitized derived payload, and pc-res151-primary-stat-lookup.json binds its source hashes, manifest and original-runtime checks. The planner and agent CLI now accept resource 151 for plan validation, primary and advancement lookup, and known-component assembly. This supplies no final battle-property assembly or gameplay verification.

Android resource 83 remains a distinct, unsupported combat build. Static ARM64 tracing now recovers the exact main-Lua startup order (`gamelauncher`, `config`, `foundation`, `gamescript`, `share`, `vue`, `ejoysdk_lua`, then language), and the seven base files exactly match the packaged Scripts manifest. `LuaAssetBundlesMgr` checks the persistent default download directory first, then an Android App Bundle container mapping with a byte offset, then StreamingAssets. Lua modules select bundles by their lowercased first path component and load `Assets/Lua/<module>.lua.bytes`. A whole-binary direct-call scan finds the generated XLua wrapper as `FixUrlRoot`'s only direct ARM64 caller; packaged startup Lua has zero string-constant references to that bridge or `DownloadHelper`. See `research/evidence/android-lua-bundle-loader-inspection.json`, `research/evidence/android-url-bridge-inspection.json` and `docs/PC_ANDROID_COMPARISON.md`. The actual resource-83 Scripts bytes, concrete device persistent-data root and server endpoint remain absent; no Android formula parity or gameplay credit follows.

For a new blind holdout, follow `docs/LIVE_HOLDOUT_CAPTURE.md`. The public `tools/capture_live_replay_session.py` now creates the ignored private baseline and same-process delta without printing replay identifiers; the identifier-free commitment, review and freeze tools remain separate so their Git chronology is auditable.
...
Use `tools/freeze_gameplay_prediction.mjs` for manually reconstructed scenarios. For native replays, `tools/freeze_blind_replay_prediction.mjs` selects the first supported deterministic direct hit without using its damage amount, critical result, HP loss or Block loss. It freezes target/caster/skill identity as a disclosed condition, so the holdout tests damage resolution rather than target selection. Current-build replay claims require reviewed same-session capture evidence. The tool writes a non-overwritable scenario/evidence/freeze set, and commits must precede `tools/reveal_blind_replay_prediction.mjs`. The reveal tool refuses to inspect the selected result unless the freeze, scenario and every referenced pre-outcome evidence artifact are committed unchanged at `HEAD`. Both paths verify the current runtime and fail closed on partial or chance-dependent cases.
