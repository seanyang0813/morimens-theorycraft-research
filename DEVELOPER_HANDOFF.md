...
Resource 151 is supported only by the narrow replay-embedded, complete-live-property Active adapter. `research/evidence/pc-res151-replay-adapter-compatibility.json` composes the exact six-module byte-equality boundary with five Lua-semantically equivalent config tables. A resource-151 prediction freeze must carry that report plus `pc-res144-to-res151-combat-build.json`. Do not route general theorycraft requests or local catalog loading to resource 151.
