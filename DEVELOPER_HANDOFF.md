...
`engine/prepared-snapshot-active-skill.mjs` is the strict catalog-to-property bridge for a catalog-tagged Awakener card with one unconditional ordinary Active row. It derives tags, ordinary card context, base value and repeat count from the build-pinned exports, then calls the complete-property sequence runner. It deliberately rejects monster intents, mixed commands, `ParaPlus`, nonordinary subtypes and implicit critical outcomes. See `docs/PREPARED_SNAPSHOT_ACTIVE_SKILL.md`.
