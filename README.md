...
The live PC download has advanced to resource 150 / build 51. Its actual runtime matches resource 144 on 2,262 utility fixtures, 879 ordinary PvE offensive-assembly fixtures, 2,097 no-card target/critical fixtures, 506 card and Strike/Ulti-tag target fixtures, 40 selected property cases, 18 BeHit-through-HP cases, 16 Active routing cases and 24 narrow command-entry cases. Active effect initialization has one confirmed build-specific change: resource 150 clamps a nonpositive base repeat before applying repeat modifiers. Both catalogs contain 69 Awake skills and zero directly linked damage rows; their nested state/resource command graphs remain separate simulator work. Record and scheduler paths still require revalidation before a current replay can serve as an end-to-end holdout. See `docs/CURRENT_PC_BUILD.md` and `docs/AWAKE_CARD_COMMANDS.md`.
