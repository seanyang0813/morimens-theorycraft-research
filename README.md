...
The native decoder has passed a synthetic round trip and 43 authorized server replay containers. The index/adapter have synthetic integration coverage and 428 retrospective exact gameplay consistency checks across 42 unique batch records. `tools/audit_replay_batch.mjs` reruns the fail-closed aggregate audit from private decoded inputs while publishing only neutral labels, counts and hashes. `docs/REPLAY_ACQUISITION_PATH.md` traces the authenticated client dependency from player record lookup to downloaded JSON without storing a player identifier. See `docs/LOCAL_REPLAY_RECOVERY.md`, `docs/REPLAY_ACTION_CANDIDATE.md` and `docs/BLIND_REPLAY_HOLDOUT.md` for the explicit limits.
