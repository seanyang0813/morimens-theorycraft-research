...
The local website exposes these operations in `wheel-events.html`. Users can edit the exact JSON state, load one example for each recovered event family, and inspect the verified-runtime transition without turning the result into a damage claim.
