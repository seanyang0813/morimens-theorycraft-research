...
The live PC download has advanced to resource 150 / build 51. The actual resource-150 utility formula matches all 2,262 resource-144 fixtures, its no-card final target/critical path matches all 2,097 inherited fixtures, its none/Strike/Ulti/combined-tag card extension matches 506 cases, 40 selected property cases match, and the narrow ordinary PvE command-entry gate matches 24 inherited cases. Active effect initialization has one confirmed build-specific change: resource 150 clamps a nonpositive base repeat before applying repeat modifiers. Offensive-input assembly and the remaining changed routing, record and scheduler paths require separate revalidation before a current replay can serve as an end-to-end holdout. See `docs/CURRENT_PC_BUILD.md`.
