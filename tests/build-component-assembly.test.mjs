...
const plan=()=>({schemaVersion:1,kind:'morimens-build-plan',catalogRevision:catalog.source.revision,clientBuild:'pc-res144-build51',team:[{slotId:'one',characterId:resolvableCharacter.id,level:90,gnosticRank:5,wheelId:wheel.id,wheelEnhanceLevel:15}]});
...
test('known build assembler emits an auditable contribution ledger without final damage',()=>{
  ...
  assert.deepEqual(result.members[0].primaryStats,Object.fromEntries(result.members[0].contributionLedger.slice(0,3).map(row=>[row.property,row.value])));
...
test('unknown build inputs produce typed issues and are never neutralized',()=>{
  const input=plan();delete input.clientBuild;input.team[0].level=null;input.team[0].gnosticRank=null;input.team[0].wheelId=null;delete input.team[0].wheelEnhanceLevel;
  ...
  assert.deepEqual(result.issues.map(row=>row.code),['CLIENT_BUILD_REQUIRED','CHARACTER_LEVEL_REQUIRED','GNOSTIC_RANK_REQUIRED','WHEEL_SELECTION_REQUIRED']);
});
