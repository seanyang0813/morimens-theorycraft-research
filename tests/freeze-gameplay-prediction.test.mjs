...
test('prediction freeze pins scenario, runtime and separate pre-outcome evidence without overwrite',()=>{
  ...
  try{
    ...
    const unsupportedReport=join(directory,'unsupported-build.json');writeFileSync(unsupportedReport,JSON.stringify({schemaVersion:1,kind:'MORIMENS_PC_COMBAT_BUILD_COMPARISON',currentBuild:'pc-res154-build51',currentVersion:{resVersion:154,buildVersion:51},sourceHashes:{versionManifest:'e'.repeat(64),bundles:{'share.ab':{sha256:'f'.repeat(64),size:1}}}}));
    assert.throws(()=>freezeGameplayPrediction({...args,outputFile:relativeToRoot(join(directory,'unsupported-build-freeze.json')),recordedCombatBuild:'pc-res154-build51',buildEvidenceFiles:[relativeToRoot(unsupportedReport)]}),/not supported by the prediction runtime/);
