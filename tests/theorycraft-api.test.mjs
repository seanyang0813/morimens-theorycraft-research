...
test('agent API advertises explicit bounded operations',()=>{
  ...
  assert.deepEqual(response.result.resource151OperationScope,['calculate-snapshot-active-damage','run-snapshot-active-sequence','prepare-skill-command','run-prepared-snapshot-active-skill','run-ulti-energy-effect','run-card-resource-timeline','run-prepared-snapshot-block-skill','run-prepared-state-active-sequence','run-prepared-state-active-chain','run-paid-prepared-state-active-chain','advance-after-use-card-wheel-trigger','advance-after-keeper-skill-wheel-trigger','advance-after-pursuit-wheel-triggers','run-wheel-event-sequence','run-wheel-active-timeline','compare-wheel-active-timelines','run-paid-wheel-active-timeline','run-prepared-paid-wheel-active-timeline','search-paid-wheel-orders']);
...
test('agent API exposes every real Mortal Blast row and refuses partial execution',()=>{
  const read=name=>{const bytes=readFileSync(new URL(`../research/observations/current-res150-build51/modules/${name}.json`,import.meta.url));return {data:JSON.parse(bytes),sha256:createHash('sha256').update(bytes).digest('hex')};};
  const loaded=Object.fromEntries(['Skill','BattleApi','Cmd','State'].map(name=>[name,read(name)]));const skillCommandData={build:'pc-res150-build51',skills:loaded.Skill.data,battleApi:loaded.BattleApi.data,commands:loaded.Cmd.data,states:loaded.State.data,sourceHashes:Object.fromEntries(Object.entries(loaded).map(([name,row])=>[name,row.sha256]))};
  const requestValue=JSON.parse(readFileSync(new URL('../research/examples/theorycraft-current-mouchette-mortal-blast-inspection.json',import.meta.url)));
...
test('agent API pays before exposing catalog-backed chain effects',()=>{
  const read=name=>{const bytes=readFileSync(new URL(`../research/extracted/config/${name}.json`,import.meta.url));return {data:JSON.parse(bytes),sha256:createHash('sha256').update(bytes).digest('hex')};};
  const skill=read('Skill'),battleApi=read('BattleApi'),command=read('Cmd'),state=read('State'),skillCommandData={build:'pc-res144-build51',skills:skill.data,battleApi:battleApi.data,commands:command.data,states:state.data,sourceHashes:{Skill:skill.sha256,BattleApi:battleApi.sha256,Cmd:command.sha256,State:state.sha256}};
  const requestValue=JSON.parse(readFileSync(new URL('../research/examples/theorycraft-paid-prepared-state-active-chain.json',import.meta.url)));
  const response=runTheorycraftRequest(requestValue,{skillCommandData});
  assert.equal(response.analysisTrack,'theorycrafting');assert.equal(response.result.acceptedCards,6);assert.equal(response.result.energyAfter,0);assert.equal(response.result.modeledHpLost,15);assert.equal(response.result.finalDamage,null);
});
