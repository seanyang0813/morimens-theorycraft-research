...
const base=()=>({targetBattleTag:null,targetStateIds:[],targetHasBuff:false,targetHasDebuff:false});
...
test('all ten configured state-damage rules and the shared barrier state derive from live IDs',()=>{
  const stateIds=[3469,2934,3068,2564,3792,80445,89575,90659,90650,126417,3638],result=resolveTargetDamageEligibility({...base(),targetStateIds:stateIds,targetHasBuff:true,targetHasDebuff:true});
...
test('unknown states are inert and malformed capture context fails closed',()=>{
  assert.deepEqual(resolveTargetDamageEligibility({...base(),targetStateIds:[999999]}).targetStateDamageProperties,[]);
