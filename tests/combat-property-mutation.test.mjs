...
test('ordinary and tentacle-gated combat properties match 1806 original mutation chains',()=>{
  const evidence=JSON.parse(readFileSync(new URL('./synthetic/original-combat-property-mutation.json',import.meta.url),'utf8'));
  assert.equal(evidence.fixtures.length,1806);
...
test('installed resource 150 reproduces the expanded combat-property fixture domain',()=>{
  assert.equal(current.status,'EXACT_MATCH_IN_FIXTURE_DOMAIN');assert.equal(current.fixtures,1806);assert.equal(current.exactMatches,1806);assert.deepEqual(current.mismatches,[]);
});
