...
test('Android IL2CPP update inventory recovers a relative download tree without inventing an endpoint',()=>{
  ...
  assert.equal(report.downloadStorageEvidence.relativeTemplate,'/_game_data_/DownLoad/{0}');
  ...
  assert.match(report.nextEvidenceNeeded,/_game_data_\/DownLoad/);assert.ok(report.limitations.some(row=>/formula parity is not established/i.test(row)));
});
