...
test('resource 151 is identified separately with an exact tracked-module carry-forward',()=>{
  ...
  const catalogs=read('research/evidence/pc-res150-to-res151-config-catalogs.json');
  ...
  assert.equal(catalogs.status,'LUA_SEMANTICALLY_EQUIVALENT');assert.equal(catalogs.summary.luaSemanticChangedSharedRows,0);
