...
test('pipeline derives a real generated Arachne Strike command from matching catalog evidence',()=>{
  ...
  const skill=read('Skill'),battleApi=read('BattleApi'),catalogSource={build:'pc-res144-build51',skills:skill.data,battleApi:battleApi.data,sourceHashes:{Skill:skill.sha256,BattleApi:battleApi.sha256}};
  ...
  assert.equal(plan.mainCommand.cmdId,133374);assert.deepEqual(plan.catalogResolution.baseArguments,[100,5,2]);assert.deepEqual(plan.argumentBindings,{Arg1:0.5,Arg2:5,Arg3:2});assert.equal(plan.mainCommand.castRoleUid,88);
});
