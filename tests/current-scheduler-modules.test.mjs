...
test('resource-150 core scheduler, event dispatcher and fatal-damage modules retain identity while two death modules changed',()=>{
  ...
  assert.equal(evidence.data.identicalModules,9);
  assert.equal(evidence.data.changedOrMissingModules,2);
  assert.deepEqual(evidence.data.modules.map(row=>row.name),['BattleEffectServer.lua','BattleEffectMgrServer.lua','BEFunctionEffect.lua','BEAttachPostAction.lua','BESendEvent.lua','BattleLogicEvent.lua','BattleEventMgr.lua','Table.lua','BERoleDeadlyDamage.lua','BERoleDie.lua','BattleUnitMonster.lua']);
  for(const row of evidence.data.modules.slice(0,9)){assert.equal(row.status,'IDENTICAL');assert.deepEqual(row.current,row.baseline);}
  assert.deepEqual(evidence.data.modules.slice(9).map(row=>row.status),['CHANGED_OR_MISSING','CHANGED_OR_MISSING']);
});
