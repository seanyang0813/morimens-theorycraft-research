...
test('real Arachne Strike resolves its command and base arguments before copied overrides',()=>{
  ...
  assert.deepEqual(result.command.effectTypes,['BEActiveDamage','BEGainUltiEnergy','BEAttachPostAction','BEAddState']);assert.equal(result.command.rowCount,4);assert.deepEqual(result.command.rows.map(row=>row.id),['1','2','3','4']);
...
test('catalog plan fails closed on missing live state or unversioned source',()=>{
  ...
  assert.throws(()=>prepareCatalogCardCommandPlan(input,{...source,sourceHashes:{Skill:'bad',BattleApi:battleApi.sha256,Cmd:command.sha256}}),/versioned/i);
