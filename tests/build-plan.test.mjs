...
test('client build and Gnostic rank survive round trip without filling legacy unknowns',()=>{
  ...
  for(const rank of [null,0,5]){const p=plan();p.clientBuild='pc-res144-build51';p.team[0].gnosticRank=rank;assert.deepEqual(validateBuildPlan(JSON.parse(JSON.stringify(p)),catalog).plan,p);}
