...
export function runCreateCardCommand(value){
  ...
  const deckKeys=Object.hasOwn(input.deckExpression??{},'targetPos')?['cardDeck','camp','targetPos']:['cardDeck','camp'];
  if(!exact(input.deckExpression,deckKeys)||typeof input.deckExpression.cardDeck!=='string'||!input.deckExpression.cardDeck||!Number.isFinite(input.deckExpression.camp)||(Object.hasOwn(input.deckExpression,'targetPos')&&!['TOP','BOTTOM'].includes(input.deckExpression.targetPos)))throw new Error('Resolved deck expression, camp and optional supported placement required');
  ...
  const requests=input.targets.map((target,index)=>{
    ...
    const config={enternal:input.enternal??0,show:input.show===null||input.show!==0,cardArgs:useExplicit?[...input.explicitCardArgs]:[...target.createCardArgs],castRoleUid:input.castRoleUid,camp:input.deckExpression.camp,owner:target.specialOwner,performSkillId:target.performSkillId,cardTypes:[...target.cardTypes]};
    if(Object.hasOwn(input.deckExpression,'targetPos'))config.targetPos=input.deckExpression.targetPos;
    return {targetIndex:index,deck:input.deckExpression.cardDeck,cards,config};
  });
