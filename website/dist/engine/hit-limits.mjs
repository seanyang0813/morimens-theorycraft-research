...
export function resolveHitLimits(v){
 ...
 if(!v||numeric.some(k=>!Number.isFinite(v[k])||v[k]<0)||v.hp<=0||flags.some(k=>typeof v[k]!=='boolean')||Object.keys(v).some(k=>![...numeric,...flags].includes(k)))throw new Error('Explicit nonnegative inputs and a living target are required');
 ...
 return {status:'UNVERIFIED',finalDamage:null,shield,afterRetain,converted,hpLossRequest:damage,deathResistApplied,evidence:['PC144:HitLimitHelpers'],unresolvedDependencies:['Immunity and prevent eligibility','HP property mutation and callbacks','Events, statistics and death execution','Gameplay validation']};
}
