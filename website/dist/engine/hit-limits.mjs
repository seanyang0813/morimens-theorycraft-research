...
export function resolveHitLimits(v){
 ...
 if(!v||numeric.some(k=>!Number.isFinite(v[k])||v[k]<0)||v.hp<=0||flags.some(k=>typeof v[k]!=='boolean')||Object.keys(v).some(k=>![...numeric,...flags,'build'].includes(k))||(v.build!==undefined&&!['pc-res144-build51','pc-res150-build51','pc-res151-build51'].includes(v.build)))throw new Error('Explicit nonnegative inputs and a living target are required');
 ...
 const evidence=v.build==='pc-res151-build51'?'PC151:BattleUnitBase.BeHitHp.ExactDependencyCarryforward':v.build==='pc-res150-build51'?'PC150:BattleUnitBase.BeHitHp':'PC144:HitLimitHelpers';
 return {status:'UNVERIFIED',finalDamage:null,shield,afterRetain,converted,hpLossRequest:damage,deathResistApplied,evidence:[evidence],unresolvedDependencies:['Immunity and prevent eligibility','HP property mutation and callbacks','Events, statistics and death execution','Gameplay validation']};
}
