...
export function runPreparedSkillRequest(value,source){
  ...
  if(!source||source.build!==build||!map(source.skills)||!map(source.battleApi)||!map(source.commands)||!map(source.states)||!exact(source.sourceHashes,['Skill','BattleApi','Cmd','State'])||Object.values(source.sourceHashes).some(hash=>!(/^[0-9a-f]{64}$/i).test(hash)))throw new Error('Matching versioned Skill, BattleApi, Cmd and State context required');
