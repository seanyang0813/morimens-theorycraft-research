...
export function runGeneratedCardPipeline(value){
  ...
  const commandPlans=cards.map((card,index)=>{
    const context=input.commandContexts[index];if(!exact(context,['preCmdId','cmdId','rawSkillArguments']))throw new Error(`Explicit command context ${index} required`);
    return prepareCardCommandPlan({schemaVersion:1,kind:'morimens-card-command-plan',build:input.build,card:{uid:card.uid,tid:card.tid,level:card.level,createCardArgs:card.cardArgs,resolvedOwnerUid:card.resolvedOwnerUid},preCmdId:context.preCmdId,cmdId:context.cmdId,rawSkillArguments:context.rawSkillArguments});
  });
