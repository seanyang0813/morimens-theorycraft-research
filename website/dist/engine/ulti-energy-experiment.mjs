...
export function runUltiEnergyExperiment(value){
 ...
 if(!exact(input,['schemaVersion','kind','build','otherEvents','parameters','source','targetOrder','targets'])||input.schemaVersion!==1||input.kind!=='morimens-ulti-energy-experiment'||!['pc-res144-build51','pc-res150-build51','pc-res151-build51'].includes(input.build)||input.otherEvents!=='assumed-absent'||!Array.isArray(input.targets)||!Array.isArray(input.targetOrder))throw new Error('Explicit ultimate-energy experiment required');
