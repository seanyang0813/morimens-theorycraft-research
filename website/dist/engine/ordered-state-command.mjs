...
export function runOrderedStateCommand(value){
  ...
  const baseKeys=['schemaVersion','kind','build','otherEvents','command','variables','targetBinding','attackBase','state'],hasEnergy=Object.hasOwn(input,'energy'),hasBlock=Object.hasOwn(input,'block');
  ...
  if(rows.length<2||!rows.some(row=>['BEAddState','BERemoveState','BESubStateLayer','BEGainBlock'].includes(row.Type))||!rows.some(row=>row.Type==='BEActiveDamage'))throw new Error('At least one mutation and one damage row required');
  for(const row of rows){
    if(!['BEAddState','BERemoveState','BESubStateLayer','BEActiveDamage','BEGainUltiEnergy','BEGainBlock'].includes(row.Type))throw new Error(`Unsupported ordered row type ${row.Type??'missing'}`);
    ...
    if(!['BEActiveDamage','BEGainUltiEnergy','BEGainBlock'].includes(row.Type)&&!['CmdCaster',input.targetBinding.expression].includes(row.Target))throw new Error('State rows must target the caster or supplied damage target');
  }
  ...
  const calculation=runStateSequenceExperiment({schemaVersion:1,kind:'morimens-state-sequence',build:input.build,otherEvents:'assumed-absent',crossesTurnBoundary:false,
    ...
    attackBase:{variables:input.variables,offense,targetModifiers,targetState:input.attackBase.targetState,repeatModifiers:input.attackBase.repeatModifiers,immune:input.attackBase.immune},...(hasEnergy?{energy:input.energy}:{}),...(hasBlock?{block:input.block}:{})});
  return {schemaVersion:1,status:'EXPERIMENTAL',build:input.build,finalDamage:null,completed:calculation.completed,stop:calculation.stop,targetAfter:calculation.targetAfter,
    casterEnergyAfter:calculation.casterEnergyAfter,actorBlockAfter:calculation.actorBlockAfter,modeledHpLost:calculation.modeledHpLost,properties:calculation.properties,states:calculation.states,rowPlan:rowPlan.map(({step,...item})=>item),calculation,
    delayPolicy:'DelayTime is recorded but has no observable effect while other events are explicitly absent',
    unresolvedDependencies:[...calculation.unresolvedDependencies,'Only BEAddState, BERemoveState, BESubStateLayer, BEGainUltiEnergy, single-parameter BEGainBlock and ordinary BEActiveDamage rows against one supplied target','State, energy and Block row expressions use supplied numeric variables; live state queries and caster ultimate energy are connected for later damage rows','Live caster energy is not available to state-mutation or Block expressions','State layer subtraction excludes caster attribution','DelayTime is inert only under the explicit no-other-events assumption','No automatic targeting, build/state assembly, triggers or connected original full-command validation']};
}
