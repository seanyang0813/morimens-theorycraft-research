...
const currentBuild='pc-res151-build51';
const previousBuild='pc-res150-build51';
export const supportedDamageCategories=Object.freeze({
  [build]:['ACTIVE','PASSIVE','FIXED','PURE'],
  [previousBuild]:['FIXED','PURE'],
  [currentBuild]:['FIXED','PURE','TENTACLE'],
});
...
export const fieldsFor=type=>{
  ...
  if(type==='TENTACLE')return ['tentacleDamage','critDamagePer','beDamagePer','beDamagePer2','beDamagePer3','beTentacleDamagePer','vulnerablePer','beDamagePlus','enemyTypePer','enemyStatePer','enemyBuffPer','enemyDebuffPer','enemyBlockPer','enemyBarrierPer','paraPlus'].map(key=>({
    key,label:{tentacleDamage:'Resolved Tentacle effect damage (after command ceiling)',critDamagePer:'Resolved Tentacle Crit DMG bonus (%)',beTentacleDamagePer:'Target Tentacle damage taken (%)',vulnerablePer:'Vulnerability (%)',beDamagePlus:'Target flat damage taken',paraPlus:'Command flat damage'}[key]??key,
    defaultValue:key==='tentacleDamage'?null:0,
    group:['tentacleDamage','critDamagePer','beTentacleDamagePer','vulnerablePer'].includes(key)?'Tentacle hit':'Advanced Tentacle target',
  }));
...
export function buildGeneralScenario({clientBuild=build,damageType,values,isCrit,hitEnabled,hitValues,immune,puncture,preventEligible}){
  if(!supportedDamageCategories[clientBuild]?.includes(damageType))throw new Error('This damage category is unsupported for the selected client build');
  const scenario={build:clientBuild,mode:'experimental',damageType};
  ...
  }else if(damageType==='TENTACLE'){
    if(typeof isCrit!=='boolean')throw new Error('Choose whether this hit is critical');
    scenario.tentacle={build:clientBuild,isCrit};
    for(const f of fields)scenario.tentacle[f.key]=read(f.key);
  }else{
    const effect=Object.fromEntries(fields.map(f=>[f.key,read(f.key)]));
    Object.assign(effect,{build:clientBuild,targetDead:false});
  ...
  if(hitEnabled){
    if(clientBuild!==build)throw new Error('Shield and HP resolution is only available for the historical resource-144 calculator');
