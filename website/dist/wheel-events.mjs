export function startWheelEventLab({runWheelEventSequence,advanceDoomsdayAfterUseCard,advanceLightOfIntellectAfterKeeperSkill,advanceArachneAfterPursuit,runtimeFingerprint},doc=document){
  ...
  const render=input=>{
    ...
    else summary=`${result.trace.length} events · final Strike flat ${result.finalState.doomsday?.strikecardDamagePlus??'not tracked'} · final team amplification ${result.finalState.arachne?.basicDamagePer??'not tracked'}`;
