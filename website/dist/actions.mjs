export function startActions({runCardActionTimeline,syntheticCardActionExample,runDamageEnergyCommand,runTerminalStateCommand,syntheticDamageEnergyExample,runtimeFingerprint},doc=document){
  ...
  function run(){
    ...
    try{
      const input=JSON.parse(el('action-input').value),mixed=input.kind==='morimens-damage-energy-command',terminal=input.kind==='morimens-terminal-state-command',command=mixed||terminal;
      const result=terminal?runTerminalStateCommand(input):mixed?runDamageEnergyCommand(input):runCardActionTimeline(input);
      el('energy-heading').textContent=command?'Ultimate energy':'Card energy';
      if(command){
  ...
  el('reverse').onclick=()=>{
    try{const input=JSON.parse(el('action-input').value);if(['morimens-damage-energy-command','morimens-terminal-state-command'].includes(input.kind)){const rows=Object.values(input.command.data_list).reverse();input.command.data_list=Object.fromEntries(rows.map((row,i)=>[String(i+1),row]));}else{if(!Array.isArray(input.steps))throw new Error('Action steps required');input.steps.reverse();}el('action-input').value=JSON.stringify(input,null,2);run();}
