...
function render(){
  ...
  for(const [i,member] of team.entries()){
    ...
    section.append(...labelled('Character',select(catalog.characters,member.characterId,v=>{member.characterId=v;member.gnosticRank=null;render();},'Choose character'),member.slotId+'-character'));
    ...
    function updatePrimary(){
      ...
      try{const r=resolveClientBuildPrimary({build:clientBuild,characterId:member.characterId,level:member.level,gnosticRank:member.gnosticRank},clientData);primary.textContent=`Primary base stats: CON ${r.stats.CON} · ATK ${r.stats.ATK} · DEF ${r.stats.DEF}. Excludes Soulforge, equipment and battle effects.`;trace.textContent=JSON.stringify(r,null,2);details.hidden=false;}catch(e){primary.textContent=e.message;}
    }
...
$('add').addEventListener('click',()=>{while(team.some(m=>m.slotId==='slot-'+nextId))nextId++;team.push({slotId:'slot-'+nextId++,characterId:null,level:null,wheelId:null,wheelEnhanceLevel:null});render();invalidate();});
