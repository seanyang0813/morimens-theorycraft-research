...
function render(){
  ...
  for(const [i,member] of team.entries()){
    ...
    const clientCharacter=selectedClientData()?.characters.find(row=>row.characterId===member.characterId),talents=clientCharacter?.advancementTalents??[];
...
try{const responses=await Promise.all([fetch('build-catalog.json'),fetch('client-build-data.json'),fetch('client-build-data-res150.json')]);if(responses.some(r=>!r.ok))throw new Error('Could not load build data');const [loadedCatalog,historical,current]=await Promise.all(responses.map(r=>r.json()));catalog=loadedCatalog;clientDataByBuild={[historical.build]:historical,[current.build]:current};for(const id of ['add','save','load','assemble'])$(id).disabled=false;$('add').click();}catch(e){$('error').textContent=e.message;}
