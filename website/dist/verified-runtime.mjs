...
export async function loadVerifiedRuntime({fetchFile=path=>fetch(path,{cache:'no-store'}),digest=bytes=>crypto.subtle.digest('SHA-256',bytes),makeUrl=source=>URL.createObjectURL(new Blob([source],{type:'text/javascript'})),revokeUrl=url=>URL.revokeObjectURL(url),importModule=url=>import(url)}={}){
  ...
  for(const name of names){
    if(!/^(engine\/[a-z0-9-]+\.mjs|build-catalog\.json|client-build-data(?:-res150|-res151)?\.json|wheel-mechanics-capability-catalog\.json|wheel-current-compatibility\.json|scenario\.json|rules\.json)$/.test(name))throw new Error('Unsupported runtime asset path');
  ...
  try{
    ...
    const snapshotBranches=await importModule(moduleUrl('engine/snapshot-active-branches.mjs'));
    const snapshotSequence=await importModule(moduleUrl('engine/snapshot-active-sequence.mjs'));
    const directTentacle=await importModule(moduleUrl('engine/tentacle-direct-command.mjs'));
    const wheelTriggers=await importModule(moduleUrl('engine/wheel-trigger-transitions.mjs'));
    const wheelSequence=await importModule(moduleUrl('engine/wheel-event-sequence.mjs'));
    const wheelActive=await importModule(moduleUrl('engine/wheel-active-timeline.mjs'));
    const wheelActiveComparison=await importModule(moduleUrl('engine/wheel-active-comparison.mjs'));
    const paidWheelActive=await importModule(moduleUrl('engine/paid-wheel-active-timeline.mjs'));
    const paidWheelSearch=await importModule(moduleUrl('engine/paid-wheel-order-search.mjs'));
    return {calculateDirectTentacleCommand:directTentacle.calculateDirectTentacleCommand,searchPaidWheelOrders:paidWheelSearch.searchPaidWheelOrders,runPaidWheelActiveTimeline:paidWheelActive.runPaidWheelActiveTimeline,compareWheelActiveTimelines:wheelActiveComparison.compareWheelActiveTimelines,runWheelActiveTimeline:wheelActive.runWheelActiveTimeline,runWheelEventSequence:wheelSequence.runWheelEventSequence,advanceDoomsdayAfterUseCard:wheelTriggers.advanceDoomsdayAfterUseCard,advanceLightOfIntellectAfterKeeperSkill:wheelTriggers.advanceLightOfIntellectAfterKeeperSkill,advanceArachneAfterPursuit:wheelTriggers.advanceArachneAfterPursuit,calculateSnapshotActiveDamage:snapshot.calculateSnapshotActiveDamage,calculateSnapshotActiveBranches:snapshotBranches.calculateSnapshotActiveBranches,runSnapshotActiveSequence:snapshotSequence.runSnapshotActiveSequence,runOrderedStateCommand:ordered.runOrderedStateCommand,runTerminalStateCommand:terminal.runTerminalStateCommand,runDamageEnergyCommand:mixed.runDamageEnergyCommand,syntheticDamageEnergyExample:mixedExample.syntheticDamageEnergyExample,runResearchTimeline:timeline.runResearchTimeline,compareTimelines:comparison.compareTimelines,runCardActionTimeline:actions.runCardActionTimeline,searchCardOrders:orderSearch.searchCardOrders,syntheticCardActionExample:examples.syntheticCardActionExample,runStateSequenceExperiment:states.runStateSequenceExperiment,syntheticStateSequenceExample:stateExample.syntheticStateSequenceExample,runtimeFingerprint:manifest.fingerprint};
