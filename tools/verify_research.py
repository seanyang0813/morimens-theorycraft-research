...
def main():
    ...
    if tentacle_replay.get('analysisTrack')!='verification' or tentacle_replay.get('recordedCombatBuild') is not None or tentacle_replay.get('status')!='EXACT_RETROSPECTIVE_PREHIT_COMPONENT_MATCHES' or tentacle_summary.get('distinctSourceReplays')!=3 or tentacle_summary.get('noncritEligible')!=19 or tentacle_summary.get('exact')!=19 or tentacle_summary.get('mismatch')!=0:raise ValueError('Retrospective Tentacle replay boundary is invalid')
    ...
    out['tentacleReplayComponentConsistency']={'report':str(tentacle_replay_path.relative_to(ROOT)),'reportSha256':hashlib.sha256(tentacle_replay_path.read_bytes()).hexdigest(),'analysisTrack':'verification','distinctReplays':3,'distinctCardActions':19,'exactPreHitComparisons':19,'mismatches':0,'recordedCombatBuild':None,'publicationCredit':False}
