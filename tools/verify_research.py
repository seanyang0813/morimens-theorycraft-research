...
def main():
    ...
    if tentacle_replay.get('analysisTrack')!='verification' or tentacle_replay.get('recordedCombatBuild') is not None or tentacle_replay.get('status')!='EXACT_RETROSPECTIVE_PREHIT_COMPONENT_MATCHES' or tentacle_summary.get('distinctSourceReplays')!=4 or tentacle_summary.get('noncritEligible')!=34 or tentacle_summary.get('exact')!=34 or tentacle_summary.get('mismatch')!=0 or tentacle_summary.get('critEligible')!=5 or tentacle_summary.get('internationalCritBranchExact')!=5 or tentacle_summary.get('japanCritBranchExact')!=0:raise ValueError('Retrospective Tentacle replay boundary is invalid')
    ...
    out['tentacleReplayComponentConsistency']={'report':str(tentacle_replay_path.relative_to(ROOT)),'reportSha256':hashlib.sha256(tentacle_replay_path.read_bytes()).hexdigest(),'analysisTrack':'verification','distinctReplays':4,'distinctCardActions':39,'exactPreHitComparisons':34,'criticalBranchComparisons':5,'internationalBranchMatches':5,'japanBranchMatches':0,'mismatches':0,'recordedCombatBuild':None,'publicationCredit':False}
