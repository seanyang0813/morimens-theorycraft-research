...
def main():
    ...
    fixed_replay_path=ROOT/'research/evidence/fixed-replay-consistency.json'
    fixed_replay=json.loads(fixed_replay_path.read_text(encoding='utf-8'))
    if fixed_replay.get('analysisTrack')!='verification' or fixed_replay.get('recordedCombatBuild') is not None or fixed_replay.get('selection',{}).get('distinctReplays')!=1 or fixed_replay.get('comparison',{}).get('exact')!=85 or fixed_replay.get('comparison',{}).get('mismatches')!=0:raise ValueError('Retrospective Fixed replay boundary is invalid')
    tentacle_replay_path=ROOT/'research/evidence/tentacle-replay-simple-consistency.json'
    tentacle_replay=json.loads(tentacle_replay_path.read_text(encoding='utf-8'))
    tentacle_summary=tentacle_replay.get('summary',{})
    if tentacle_replay.get('analysisTrack')!='verification' or tentacle_replay.get('recordedCombatBuild') is not None or tentacle_replay.get('status')!='EXACT_RETROSPECTIVE_PREHIT_COMPONENT_MATCHES' or tentacle_summary.get('distinctSourceReplays')!=4 or tentacle_summary.get('noncritEligible')!=34 or tentacle_summary.get('exact')!=34 or tentacle_summary.get('mismatch')!=0 or tentacle_summary.get('critEligible')!=5 or tentacle_summary.get('internationalCritBranchExact')!=5 or tentacle_summary.get('japanCritBranchExact')!=0 or tentacle_summary.get('composedDirectCommandMatches')!=39:raise ValueError('Retrospective Tentacle replay boundary is invalid')
    tentacle_team_path=ROOT/'research/evidence/pc-res151-tentacle-team-aggregation.json'
    tentacle_team=json.loads(tentacle_team_path.read_text(encoding='utf-8'))
    if tentacle_team.get('build')!='pc-res151-build51' or tentacle_team.get('status')!='EXACT_IN_SYNTHETIC_FIXTURE_DOMAIN' or tentacle_team.get('originalRuntimeCases')!=346:raise ValueError('Installed Tentacle team aggregation boundary is invalid')
    ...
    out['tentacleReplayComponentConsistency']={'report':str(tentacle_replay_path.relative_to(ROOT)),'reportSha256':hashlib.sha256(tentacle_replay_path.read_bytes()).hexdigest(),'analysisTrack':'verification','distinctReplays':4,'distinctCardActions':39,'exactPreHitComparisons':34,'criticalBranchComparisons':5,'internationalBranchMatches':5,'japanBranchMatches':0,'composedDirectCommandMatches':39,'mismatches':0,'recordedCombatBuild':None,'publicationCredit':False}
    out['tentacleTeamAggregation']={'report':str(tentacle_team_path.relative_to(ROOT)),'reportSha256':hashlib.sha256(tentacle_team_path.read_bytes()).hexdigest(),'syntheticOriginalRuntimeCases':346,'publicationCredit':False}
    path=ROOT/'research/evidence/verification-snapshot.json'
    path.write_text(json.dumps(out,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
    print(json.dumps({'suitePassed':out['automatedSuite']['passed'],'testFiles':len(tests),'realObservationRecords':len(rows),'mechanicRegressionExactMatches':out['gameplayMechanicRegressions']['exactMatches'],'resource150CatalogReplayMatches':out['replayCatalogAttribution']['resource150Matches'],'retrospectiveReplayCandidates':out['retrospectiveReplayCorpus']['activeCandidates'],'retrospectiveFixedComparisons':out['fixedReplayComponentConsistency']['exactPreHitComparisons'],'retrospectiveTentacleComparisons':out['tentacleReplayComponentConsistency']['exactPreHitComparisons'],'reviewablePredictions':reviewable,'reviewableHoldouts':holdouts,'publicationStatus':out['publicationStatus'],'report':str(path.relative_to(ROOT))},indent=2))
