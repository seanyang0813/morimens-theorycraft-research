...
def main():
    ...
    for case in fixture['fixtures']:
        actual=oracle.run_case(case['input'])
        if actual!=case['expected']:mismatches.append({'input':case['input'],'baseline':case['expected'],'current':actual})
    report={'schemaVersion':1,'kind':'MORIMENS_PC_CROSS_BUILD_RUNTIME_COMPARISON','baselineBuild':'pc-res144-build51','currentBuild':'pc-res150-build51','method':'BattleRecord selected frame constructors','status':'EXACT_MATCH_IN_FIXTURE_DOMAIN' if not mismatches else 'BEHAVIOR_CHANGE_DETECTED','sourceHashes':{'comparison':sha(comparison_path),'fixture':sha(fixture_path),'currentBattleRecord':sha(module)},'fixtures':len(fixture['fixtures']),'exactMatches':len(fixture['fixtures'])-len(mismatches),'mismatches':len(mismatches),'results':mismatches,'scope':'Actual resource-150 selected BattleRecord card-use, hit, property, target and state frame constructors.','limitations':['No record queue, serialization, replay playback, gameplay or holdout credit']}
