...
NAMES=("BattleEffectServer.lua","BattleEffectMgrServer.lua","BEFunctionEffect.lua","BEAttachPostAction.lua","BESendEvent.lua","BattleLogicEvent.lua","BattleEventMgr.lua","Table.lua","BERoleDeadlyDamage.lua","BERoleDie.lua","BattleUnitMonster.lua")
...
def main():
    ...
    identical=sum(row['status']=='IDENTICAL' for row in rows);core_identical=all(row['status']=='IDENTICAL' for row in rows[:8])
