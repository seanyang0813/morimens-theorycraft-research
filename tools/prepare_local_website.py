...
files={name:hashlib.sha256((DEST/name).read_bytes()).hexdigest() for name in sorted([*['engine/'+n for n in modules],'build-catalog.json','client-build-data.json','client-build-data-res150.json','wheel-mechanics-capability-catalog.json','scenario.json','rules.json'])}
