...
export function revealBlindReplayPrediction({indexFile,freezeFile,observationFile}){
  ...
  const damageType=evidence.routing?.damageType??1;
  if(![1,6].includes(damageType)||observed?.damageType!==damageType)throw new Error('Revealed damage category differs from frozen routing');
  ...
  const observation={id:evidence.holdoutId,kind:'REAL_GAME_OBSERVATION',status:buildConfirmed?'COMPLETE':'COMPLETE_BUILD_UNCONFIRMED',holdout:true,source:'Authorized selected-profile native replay; identifiers withheld',version:{viewingClient:recordedCombatBuild,recordedCombatBuild},visibleState:{sourceActionIndex:evidence.sourceActionIndex,sourceHitIndex:evidence.sourceHitIndex,damageType,skillId:evidence.routing.skillId,commandId:evidence.routing.commandId,rowId:evidence.routing.rowId,eligibleRowIds:evidence.routing.eligibleRowIds??[evidence.routing.rowId],repetitionPerExecution:evidence.routing.repetitionPerExecution},assumedState:{},uncertainState:buildConfirmed?[]:['Recorded engine-code version is not explicit in the replay payload'],observedDamage:observed.castDamage,predictedDamage:predicted,difference,mechanicsExercised:[damageType===6?'Deterministic Fixed pre-hit damage':'Deterministic Active damage','Replay-embedded combat catalogs','First-hit identity-conditional prediction'],prediction:freeze.prediction,predictionFrozenBeforeOutcomeEvidence:{path:rel(freezePath),sha256:hash(readFileSync(freezePath))},evidence:[...evidenceItems.map(item=>item.path),...(freeze.recordedBuild?.evidence??[]).map(item=>item.path),rel(outcomePath)]};
