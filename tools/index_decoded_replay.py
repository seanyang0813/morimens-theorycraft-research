...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--input',required=True,type=Path);parser.add_argument('--output',required=True,type=Path);parser.add_argument('--strategy-summary-output',type=Path);parser.add_argument('--strategy-label');parser.add_argument('--wave',type=int);parser.add_argument('--difficulty');parser.add_argument('--include-outcomes',action='store_true');args=parser.parse_args()
    ...
    if args.strategy_summary_output:
        ...
        summary=build_strategy_summary(index,artifact.get('decoded',{}).get('resourceRecords',{}),label=args.strategy_label,wave=args.wave,difficulty=args.difficulty,include_outcomes=args.include_outcomes)
