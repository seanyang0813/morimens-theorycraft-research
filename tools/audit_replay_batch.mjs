...
while(args.length){
  ...
  if(!value||!['--replay','--output','--combat-build'].includes(flag))throw new Error('Usage: node tools/audit_replay_batch.mjs --replay id,index,decoded,container [--replay ...] [--combat-build BUILD] --output FILE');
  if(flag==='--output')output=value;else if(flag==='--combat-build')combatBuild=value;else specs.push(value);
}
if(!output||!specs.length)throw new Error('At least one replay and an output path are required');
if(!['pc-res144-build51','pc-res150-build51'].includes(combatBuild))throw new Error('Unsupported combat build');
