...
def main():
    ...
    parser.add_argument('--json-encoding',default='utf8',choices=['utf8','latin1'],help='Encoding of the JSON container bytes; server replay responses use latin1-compatible binary JSON')
    ...
    compressed=container_compstr(source,args.encoding,args.json_encoding);decoded=ReplayCodec().decode(compressed)
    artifact={'schemaVersion':1,'kind':'MORIMENS_DECODED_REPLAY','inputSha256':hashlib.sha256(source.read_bytes()).hexdigest(),'jsonEncoding':args.json_encoding,'compStrEncoding':args.encoding,'decoded':decoded}
