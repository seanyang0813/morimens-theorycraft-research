...
def main():
    ...
    for name in ('BattleUnitBase.lua','BattleUnitUtil.lua'):
        if combat[name]['status']!='IDENTICAL':raise ValueError(f'{name} equality required')
    ...
    if scheduler['BattleLogicEvent.lua']['status']!='IDENTICAL':raise ValueError('BattleLogicEvent.lua equality required')
    ...
    report={'schemaVersion':1,'kind':'MORIMENS_PC_CROSS_BUILD_RUNTIME_COMPARISON','baselineBuild':'pc-res144-build51','currentBuild':'pc-res150-build51','method':'selected skill-phase command/parser boundary','status':'EXACT_MATCH_IN_FIXTURE_DOMAIN' if not mismatch_rows else 'BEHAVIOR_CHANGE_DETECTED','sourceHashes':hashes,'fixtures':total,'exactMatches':exact,'mismatches':len(mismatch_rows),'domains':domains,'results':mismatch_rows,'scope':'Resource-150 BattleCmdServer TriggerCmd through effect-list construction, original GenerateEffectObj to original effect-manager creation, real BEActiveDamage and BEFunctionEffect construction/target and parameter binding/initialization/repetition routing/function dispatch into Damage2SingleTarget, GetRealDmg arithmetic, BattleUnitBase.BeHit/BattlePropertyServer HP mutation, nonlethal event-request ordering and one lethal monster death/kill-request boundary, phase cleanup, skill arguments, delay planning and row ordering plus BattleCmdParser StateOwner selection, target handoff and ArgN lookup.','limitations':['Connected hit fixtures use explicit neutral PvE actor/player/target adapters and empty skill types; property-send/hit/event-factory callbacks are observers and animation is a no-op adapter. BERoleDeadlyDamage and BERoleDie constructors are request observers only; no death-effect body, event listener execution, target-expression initialization, complete card phase, gameplay or holdout credit']}
