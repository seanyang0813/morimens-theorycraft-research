...
from card_command_init_oracle import CardCommandInitOracle, CASES as COMMAND_CASES
...
def main():
    ...
    create=CreateCardOracle(overrides);add=AddNewCardOracle({**overrides,'BattleLogicEvent':asset(logic)});owner=CardOwnerOracle(overrides);command=CardCommandInitOracle(overrides)
    domains=[('create',CREATE_CASES,create.evaluate,ROOT/'tests/synthetic/original-create-card.json'),('add',ADD_CASES,add.run,ROOT/'tests/synthetic/original-add-new-card.json'),('owner',OWNER_CASES,owner.run_owner,ROOT/'tests/synthetic/original-card-owner.json'),('command',COMMAND_CASES,command.run_init,ROOT/'tests/synthetic/original-card-command-init.json')]
    results=[];total=0
