"""Run phase-limit property construction with changed resource-150 modules."""
import hashlib
import json
import os
...
from connected_phase_property_creation_oracle import EXPRESSION
...
def main():
    fixture_path = ROOT / 'tests/synthetic/original-connected-phase-property-creation.json'
    ...
    if changed != ['BEAddStateParent', 'BattlePropertyServer']:
        raise ValueError(f'Unexpected changed phase/property modules: {changed}')
    ...
    if installed_hashes != current_hashes:
        raise ValueError('Private phase/property module copies do not match installed share bundle')
    ...
    for index, row in enumerate(fixture['fixtures']):
        values = row['input']
        actual = oracle.run(values['stateId'], values['requestedLayer'], values['maximum'], property_value=values['resolvedLimit'])
    ...
    report = {
        ...
        'mismatches': 0,
        'scope': 'Installed changed add/property modules reproduce every inherited state-60408 layer and be_damage_limit storage fixture.',
        'limitations': [
            'Transition and property expressions are separately verified and supplied as resolved parser values',
            'Synthetic non-player target and callback observers',
            'No joined command-row execution, event body, gameplay or holdout credit',
        ],
    }
    output = ROOT / 'research/evidence/pc-res150-connected-phase-property-creation-runtime.json'
