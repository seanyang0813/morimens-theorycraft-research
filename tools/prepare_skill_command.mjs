...
if(![3,4].includes(process.argv.length)){console.error('Usage: node tools/prepare_skill_command.mjs request.json [execution-context.json]');process.exitCode=2;}
  ...
  const result=runPreparedSkillRequest({schemaVersion:1,kind:'morimens-prepared-skill-request',preparation:request,execution},{skills:skills.data,battleApi:api.data,commands:commands.data,sourceHashes:{Skill:skills.sha256,BattleApi:api.sha256,Cmd:commands.sha256}});
